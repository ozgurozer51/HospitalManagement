...
if($islem=="listeyi-getir"){ ?>
        ...
        <div class="card-body bg-white">
            ...
            <table id="birimlertab" class="table table-striped table-bordered w-100 display nawrap" >
                ...
                <tr>
                    ...
                    <th>Poliklinik Sıra No</th>
                    <th>Poliklinik Durum</th>
                    <th>İşlem</th>
                </tr>
                </thead>
                <tbody>

                <?php $sql = verilericoklucek('select * from units where unit_type="0"');
                foreach ($sql as $row){ ?>
                    <tr>
                        ...
                        <td align="center">
                            <i class="fa fa-pen-to-square poliklinik_duzenle_modal" title="Düzenle"
                                <?php if ($row['status']){ ?> id="<?php echo $row["id"]; ?>"   data-bs-target="#polikilinik-modal" data-bs-toggle="modal"  <?php } ?>
                               alt="icon"></i>

                            <?php if($row['status']=='0'){ ?>
                                <i class="fa-solid fa-recycle polaktif"
                                   title="Aktif Et" id="<?php echo $row["id"]; ?>" alt="icon" ></i>

                            <?php }else{ ?>

                                <i class="fa fa-trash poliklinik_sil_open_pop"
                                   title="İptal" id="<?php echo $row["id"]; ?>" alt="icon"></i>
    ...
    <div class="modal fade" id="polikilinik-modal" aria-hidden="true">
        <div class="modal-dialog"  id="polikilinik-modal-icerik" style=" width: 100% !important; max-width: 100% !important; ">
        </div>
    </div>

    <script>
        $(document).ready(function() {
            $("body").off("click", ".poliklinik_duzenle_modal").on("click", ".poliklinik_duzenle_modal", function(e){
                var bolumid = $(this).attr('id');
                $.get( "ajax/tanimlar/polikliniktanimla.php?islem=modal-icerik", { birimbilgisi:bolumid },function(getveri){
                    $('#polikilinik-modal-icerik').html(getveri);
                });
            });

            $("body").off("click",".polaktif").on("click",".polaktif", function (e) {
                var getir = $(this).attr('id');
                $.ajax({
                    type:'POST',
                    url:'ajax/tanimlar/polikliniktanimla.php?islem=pol-aktiflestir',
                    data:{getir},
                    success:function(e){
                        $('#sonucyaz').html(e);
                        $('.poliklinik-tanim:first').load("ajax/tanimlar/polikliniktanimla.php?islem=listeyi-getir");
                    }
                });
            });

            $("body").off("click", ".poliklinik_sil_open_pop").on("click", ".poliklinik_sil_open_pop", function(e){
            var id = $(this).attr('id');
            ...
            $('#birimlertab').DataTable( {
                ...
                buttons: [
                    {
                        text: 'Poliklinik Ekle',
                        className: 'btn btn-success btn-sm btn-kaydet',
                        attr:  {
                            'data-bs-toggle':"modal",
                            'data-bs-target':"#polikilinik-modal",
                        },
                        
                        action: function ( e, dt, button, config ) {
                            $.get( "ajax/tanimlar/polikliniktanimla.php?islem=modal-icerik",function(get){
                                $('#polikilinik-modal-icerik').html(get);
                            });
                        }
                    }
                ],
        ...
        <form class="modal-content" id="form_birim" action="javascript:void(0);">
            <div class="modal-header text-white">
                <?php if ($_GET["birimbilgisi"] != "") {
                    echo '<h4 class="modal-title">Poliklinik Düzenle</h4>';
                    $birimbilgisi = singular("units", "id", $_GET["birimbilgisi"]);

                    extract($birimbilgisi);
                } else { echo '<h4 class="modal-title">Poliklinik Ekle</h4>';} ?>
       ...
       transaction_detail.*,transaction_definitions.* FROM transaction_definitions,transaction_detail WHERE 
        transaction_definitions.definition_type='ISLEM_GRUBU' AND transaction_definitions.id=transaction_detail.transaction_id AND transaction_definitions.status='1'";
            ...
            <div class="modal-footer">
                <?php if ($_GET["birimbilgisi"] != '') { ?>
                    <input placeholder=" " class="btn btn-success btn-sm w-md justify-content-end" id="birimguncelle" type="submit" data-bs-dismiss="modal"  value="Düzenle"/>
                    <?php
                } else {?>
                    <input placeholder=" " class="btn w-md justify-content-end btn-success  btn-sm" id="birimkaydet" type="submit"  data-bs-dismiss="modal" value="Kaydet"/>
                <?php } ?>
                        ...
                        <script type="text/javascript">
                            $(document).ready(function () {
                                $("#bina").change(function () {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        url: "ajax/tanimlar/polikliniktanimla.php?islem=binaidgetir",
                                        data: {"binano": binano},
                                        success: function (e) {
                                            $("#kat").html(e);
                                        }
                ...
                $.ajax({
                    type: 'POST',
                    url: 'ajax/tanimlar/polikliniktanimla.php?islem=poliklinik-ekle',
                    data: gonderilenform,
                    success: function (e) {
                        console.log(e);
                        $("#sonucyaz").html(e);
                ...
                $.ajax({
                    type: 'POST',
                    url: 'ajax/tanimlar/polikliniktanimla.php?islem=poliklinikguncelle',
                    data: gonderilenform,
                    success: function (e) {
                        console.log(e);
                        $("#sonucyaz").html(e);
...
<?php } else if($_GET["islem"]=="poliklinik-ekle") {
    ...
    if ($birimtanimla == 1) { ?>
        <script>
            alertify.success('Poliklinik Ekleme Başarılı');
        </script>
    <?php   } else { ?>
        <script>
            alertify.error('Poliklinik Ekleme Başarısız');
        </script>
    <?php } }

else if($_GET["islem"]=="poliklinikguncelle") {
    ...
    if ($sql == 1) { ?>
        <script>
            alertify.success('Poliklinik Güncelleme Başarılı');
        </script>
    <?php   } else { ?>
        <script>
            alertify.error('Poliklinik Güncelleme Başarısız');
        </script>
    <?php } }

else if ($islem=="polikliniksil"){
    $id=$_POST['id'];
    $detay=$_POST['delete_detail'];

    $sql=canceldetail("units","id",$id,$detay);
    if ($sql == 1) { ?>
        <script>
            alertify.success('Poliklinik Silme Başarılı');
        </script>
    <?php   } else {  ?>
        <script>
            alertify.error('Poliklinik Silme Başarısız');
        </script>
    <?php } }
