...
if($tanisorgula["id"]=="")
{
    echo '<div class="alert alert-danger alert-dismissible fade show" role="alert">
                                            ...
                                            tetkik  ekleyebilmeniz için tanı girmeniz  . lütfen giriş işlemini tamamladıktan sonra tekrar deneyiniz
    ...
    if($_POST["islem"]=="tanimla"){
        ...
        $_POST["package_type"]=lab_grup_id;
        unset($_POST["islem"]);
    ...
    <div id="ihsancunedioglu">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body" >
                        ...
                        <div class="col-md-12 row">
                            <div class="col-md-6">
                                <center>
                                    <h4>Tetkik Listesi</h4>
                                </center>

                                <script>
                                    function showresult(str) {
                                        var protokolno = $(this).attr('protokolno');
                                        $.get( "ajax/istemara.php?q="+str+"&patients=<?php echo $kullanicibilgileri["patient_id"]; ?>&tip=<?php echo $_GET["tip"]; ?>&islemcik=lab", { protokolno:protokolno },function(getveri){
                                ...
                                <table id="hizlipaketlerigetirtablee<?php echo $_GET["ihsan"] ;?>" class="table table-striped table-bordered"    >
                                    ...
                                    $kullanicibilgigetir=singularactive("patients","id",$kullanicibilgileri["patient_id"]);
                                    ...
                                    where process_group.id='".lab_grup_id."' group by process_name,himet_id,hizmet_ucret,himet_kodu,hizmet_grup_id order by process_name fetch first 7 rows only";
                            ...
                            <div class="col-md-6">
                                ...
                                <table id="kullaniciisteklistesi<?php echo $_GET["ihsan"] ;?>" class="table table-striped table-bordered"    >
                                    ...
                                    $hello=verilericoklucek($sql);
                                    foreach ($hello as $row) {
                                        $istembilgileri=tekil("processes","id",$row["request_id"]);
                                        ?>
                                        <tr>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button"  id="paketiolustur<?php echo $_GET["ihsan"] ;?>" class=" btn up-btn waves-effect waves-light" data-bs-dismiss="modal">Paketi Oluştur</button>
                    </div></form>
                </div><!-- /.modal-content -->
            </div><!-- /.modal-dialog -->
        ...
        <div class="modal fade bd-example-modal" id="yeniistemekle<?php echo $_GET["ihsan"] ;?>" role="dialog">
            ...
            <div class="modal-dialog" style=" width: 75%; max-width: 75%; ">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title">Laboratuvar Listesi</h4>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body" >
                        <div class="card-body">
                                ...
                                <div class="col-md-12">
                                    <script>
                                        function showresults(str) {
                                            var protokolno = $(this).attr('protokolno');
                                            $.get( "ajax/istemara.php?q="+str+"&protokolno=<?php echo $protokolno; ?>&patientid=<?php echo $kullanicibilgileri["patient_id"]; ?>&tip=tekliekle&islemcik=lab", { protokolno:protokolno },function(getveri){
                                    ...
                                    <table id="yenipaketeklelistele<?php echo $_GET["ihsan"] ;?>" class="table table-striped table-bordered"    >
                                        ...
                                        $sql="select count(DISTINCT process_name),processes.id as hizmet_id,processes.process_name as hizmet_adi,processes_price.price as hizmet_ucret,
                                           official_code as hizmet_kodu,processes.process_group_id as hizmet_grup_id from processes
                                             inner join process_group on processes.process_group_id = process_group.id
                                             inner join processes_price on processes.id = processes_price.processes_id
                                        where process_group.id=$labid group by process_name,hizmet_id,hizmet_ucret,hizmet_kodu,hizmet_grup_id order by process_name fetch first 7 rows only";
                                        $hello=verilericoklucek($sql);
                                        foreach ($hello as $row) {
                                            ...
                                            <tr>
                                                ...
                                                <th><button type="button" islem="tekekle"  icnd="lab" kullanici_id="<?php echo $kullanici_id; ?>"   kullanici_id="<?php echo $kullanici_id; ?>"  grup_id="<?php echo $row["hizmet_grup_id"]; ?>"   protokolno="<?php echo $protokolno; ?>" tip="tekliekle"   id="<?php echo $row["hizmet_id"]; ?>"    class="istemekle<?php echo $_GET["ihsan"] ;?> btn up-btn btn-sm waves-effect waves-light">EKLE</button></th>
...
<!--                        <button --><?php //hekimeaitolayankayitsorgula($kullanicibilgileri["doctor"],$kullanicininidsi); ?><!-- type="button" data-bs-toggle="modal" data-bs-target="#hizlipaketlerigetir--><?php //echo $_GET["ihsan"] ;?><!--"  protokolno="--><?php //echo $_GET["protokolno"]; ?><!--" class="hizlipaketlistelebutonistem btn btn-warning waves-effect waves-light">-->
<!--                            <i class="bx bx-error font-size-16 align-middle me-2"></i> hızlı paket listele-->
<!--                        </button>-->
<!--                    --><?php //}    if($hizlipaketolustur){ ?>
<!--                        <button --><?php //hekimeaitolayankayitsorgula($kullanicibilgileri["doctor"],$kullanicininidsi); ?><!-- type="button"   data-bs-toggle="modal" data-bs-target="#hizlipaketolustur--><?php //echo $_GET["ihsan"] ;?><!--"   class="btn btn-warning waves-effect waves-light" style=" color: #fff; background-color: #1f3e46; border-color: #1f3e46; "> <i class="bx bx-error font-size-16 align-middle me-2"></i> hızlı paket oluştur </button>-->
<!--                    --><?php //} ?>
<!--                </div>-->
<!---->
<!--                <div class="col-3">-->
...
<!--            </div>-->
            ...
            <div class="istemlistebody mt-2">
                <script>
                    ...
                    $.get("ajax/hastadetay/hizmetlistesi.php?hizmetislem=labhizmetlistesi", {protokolno: protokolno,ihsan:ihsan}, function (getveri) {
...
<!--                    <tbody>-->
<!--                    --><?php
//                    $hastaistemlerigetir = "select patient_prompts.id as hastaistemlerid,patient_prompts.*,transaction_definitions.* from patient_prompts,transaction_definitions where patient_prompts.protocol_number='{$_GET["protokolno"]}' and patient_prompts.status='1' and patient_prompts.group_id='".lab_grup_id."' and transaction_definitions.definition_name='LAB'";
...
<!--                            <td>--><?php //if($rowa["patient_id"]!=""){ echo "evet";} ?><!--</td>-->
...
<!--                            <td>--><?php //if($odeme_yapildi!=1){?><!--<button --><?php //hekimeaitolayankayitsorgula($kullanicibilgileri["hekim"],$kullanicininidsi); ?><!-- type="button" islem="sil" protokolno="--><?php //echo $protokolno; ?><!--"   islem_id="--><?php //echo $rowa["hastaistemlerid"]; ?><!--" class="istemsillab btn btn-danger waves-effect waves-light">-->
...
<!--            </table>-->
        ...
        <script>
            ...
            $(document).ready(function () {
                ...
                $(".istemekle<?php echo $_GET["ihsan"] ;?>").click(function () {
                    ...
                    $.get("ajax/hastayaistemekle.php", {protokolno: protokolno, id: id, tip: tip,grup_id:grup_id}, function (getveri) {
                        if(getveri) {
                            ...
                            if(icnd=="istemler") {
                                $.get("ajax/hastadetay/hizmetlistesi.php?hizmetislem=istemhizmetlistesi", {protokolno: protokolno,ihsan:grup_id}, function (getveri) {
                ...
                //    alert("geldi");
                //    $.get("ajax/hastahizlipaketlistele.php?islem=lab", {protokolno: protokolno,ihsan:ihsan}, function (getveri) {
                ...
                $("#paketiolustur<?php echo $_GET["ihsan"] ;?>").on("click", function(){ // buton idli elemana tıklandığında
                    ...
                    $.ajax({
                        url:'ajax/hastadetay/hastalaboratuvarlistele.php?protokolno=<?php echo $protokolno; ?>', // serileştirilen değerleri ajax.php dosyasına
