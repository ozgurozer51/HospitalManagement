...
<div style="height:50%; max-height:50%;">
    <table id="table-<?php echo $uniqid; ?>" class="easyui-datagrid"  data-options="singleSelect:true , showRefresh:true, fitColumns:true , pagination:false" style="width:100%; height:100%; font-size: 13px;" url="ajax/formlar-raporlar/hastaya-eklenen-formlar/hasta-gecmis-rapor-form.php?islem=hasta-gecmis-islemleri&referance_table_name=<?php echo $_GET['name']; ?>&patient_id=<?php echo $_GET['patient_id']; ?>" iconCls="icon-save" rownumbers="true">
        <thead field="id">
        <tr>
            <th field="form_id">ID</th>
            <th field="insert_datetime">Eklenme Tarihi</th>
            <th field="protocol_no">Protokol No</th>
...
<script>
    ...
    $('#table-<?php echo $uniqid; ?>').datagrid({
        onSelect: function (index, row) {
            ...
            $('#ff-'+uniquid).form('load', 'ajax/formlar-raporlar/hastaya-eklenen-formlar/hasta-gecmis-rapor-form.php?islem=hasta-gecmis-form-getir&id="'+row.form_id+'"&name="'+name+'"');
    ...
    $("body").off("click", "#update-tool").on("click", "#update-tool", function (e) {
        ...
        $.ajax({
            type: 'POST',
            url: 'ajax/formlar-raporlar/hastaya-eklenen-formlar/forms-sql.php?islem=hasta-form-guncelle',
            data: form_data,
            success: function (e) {
              $(".sonucyaz").html(e);
            }
    ...
    function delete_form(){
        ...
        $.ajax({
             type: 'GET',
             url: 'ajax/formlar-raporlar/hastaya-eklenen-formlar/forms-sql.php?islem=hasta-form-sil',
    ...
    $("body").off("click", "#save-tool").on("click", "#save-tool", function (e) {
        ...
        $.ajax({
            type: 'POST',
            url:  "ajax/formlar-raporlar/hastaya-eklenen-formlar/forms-sql.php?islem=form-ekle",
            data:form ,
            success: function (e) {
                $(".sonucyaz").html(e);
                $('#table-'+uniquid).datagrid('reload');
